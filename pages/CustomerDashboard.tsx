                ...
                {steps.map((step, idx) => {
                    ...
                    return (
                        <div key={step.key} className="flex flex-col items-center gap-1 bg-white px-1">
                            <div className={`w-8 h-8 rounded-full flex items-center justify-center text-xs transition-all border-2 ${isActive
                                ? 'bg-brand-teal border-brand-teal text-white'
                                : 'bg-white border-gray-200 text-gray-300'
                                } ${isCurrent ? 'ring-2 ring-brand-light ring-offset-1' : ''}`}>
                                <step.icon size={14} />
                            </div>
                            <span className={`text-[10px] font-medium uppercase tracking-wider ${isActive ? 'text-brand-dark' : 'text-gray-300'
                                }`}>{step.label}</span>
                        </div>
                    );
                })}
                ...
                {myBookings.length === 0 ? (
                    ...
                    myBookings.map(booking => (
                        <div key={booking.id} className="bg-white p-6 rounded-xl shadow-sm border border-gray-100">
                            ...
                            <div className="border-t border-gray-100 mt-4 pt-4 flex flex-col sm:flex-row gap-4 text-sm text-gray-600 justify-between items-center">
                                ...
                                {(booking.status === 'pending' || booking.status === 'confirmed') && (
                                    <button
                                        onClick={async () => {
                                            if (window.confirm(t('booking.cancelConfirm') || 'Are you sure you want to cancel this booking?')) {
                                                try {
                                        ...
                                        className="text-red-500 hover:text-red-700 text-xs font-bold border border-red-200 px-3 py-1 rounded-full hover:bg-red-50 transition-colors"
                                    >
                                        Cancel Booking
                                    </button>
                                )}
